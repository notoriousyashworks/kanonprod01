...
async function fetchAllBelts(token) {
  const pageSize = 100;
  let allProducts = [];
  let totalPages = 1;
  ...
  for (let page = 0; page < totalPages; page++) {
    ...
    allProducts = allProducts.concat(belts);
    process.stderr.write(`Page ${page+1}/${totalPages}: ${products.length} products, ${belts.length} belts → total: ${allProducts.length}\n`);
    await sleep(300);
  }
  return allProducts;
}
...
(async () => {
  try {
    ...
    const belts = await fetchAllBelts(token);
    const slim = belts.map(p => ({
      id: p.id,
      name: p.name,
      category: p.category,
    ...
    process.stderr.write(`\nDone. Total "Belts" category products: ${belts.length}\n`);
  } catch (err) {
