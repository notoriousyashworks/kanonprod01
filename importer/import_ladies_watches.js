...
// ============================================================
// CONFIGURATION — Ladies Watches
// ============================================================
const SOURCE_URL       = 'https://indiankicks.in/shop?c=ladies-watch-watches';
const CATEGORY         = 'Ladies Watches';
const BUNNY_FOLDER     = 'kicks-aura/indiankicks/ladies-watches';
...
async function processVideo(videoFilename, productId) {
    ...
    console.log(`Creating Bunny Stream video for Ladies Watches ${productId}...`);
    const createRes = await fetch(`https://video.bunnycdn.com/library/${BUNNY_STREAM_LIBRARY_ID}/videos`, {
        ...
        body: JSON.stringify({ title: `LadiesWatches_${productId}` })
    });
...
(async () => {
    ...
    while (stats.successfullyImported < MAX_IMPORT && keepDiscovering) {
        ...
        for (let i = 0; i < toProcess.length; i++) {
            ...
            console.log(`\n[${i + 1}/${toProcess.length}] Processing Ladies Watch ${sourceId}...`);
    ...
    console.log(`- Total Ladies Watches discovered: ${stats.totalDiscovered}`);
