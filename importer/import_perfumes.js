...
// ============================================================
// CONFIGURATION — Perfumes
// ============================================================
const SOURCE_URL       = 'https://indiankicks.in/shop?c=perfume-1';
const CATEGORY         = 'Perfumes';
const BUNNY_FOLDER     = 'kicks-aura/indiankicks/perfumes';
const MAX_IMPORT       = 280;
...
async function processVideo(videoFilename, productId) {
    ...
    console.log(`Creating Bunny Stream video for Perfumes ${productId}...`);
    const createRes = await fetch(`https://video.bunnycdn.com/library/${BUNNY_STREAM_LIBRARY_ID}/videos`, {
        ...
        body: JSON.stringify({ title: `Perfumes_${productId}` })
    });
...
(async () => {
    ...
    while (stats.successfullyImported < MAX_IMPORT && keepDiscovering) {
        ...
        for (let i = 0; i < toProcess.length; i++) {
            ...
            console.log(`\n[${i + 1}/${toProcess.length}] Processing Perfume ${sourceId}...`);
    ...
    console.log(`- Total Perfumes discovered: ${stats.totalDiscovered}`);
