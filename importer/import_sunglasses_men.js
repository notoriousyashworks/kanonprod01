...
// ============================================================
// CONFIGURATION — Men's Sunglasses
// ============================================================
const SOURCE_URL       = 'https://indiankicks.in/shop?c=sunglasses-eye-wear-men';
const CATEGORY         = 'Sunglasses';
const BUNNY_FOLDER     = 'kicks-aura/indiankicks/sunglasses-men';
const MAX_IMPORT       = 50;
...
async function processVideo(videoFilename, productId) {
    ...
    console.log(`Creating Bunny Stream video for Sunglasses ${productId}...`);
    const createRes = await fetch(`https://video.bunnycdn.com/library/${BUNNY_STREAM_LIBRARY_ID}/videos`, {
        ...
        body: JSON.stringify({ title: `Sunglasses_Men_${productId}` })
    });
...
(async () => {
    ...
    while (stats.successfullyImported < MAX_IMPORT && keepDiscovering) {
        ...
        for (let i = 0; i < toProcess.length; i++) {
            ...
            console.log(`\n[${i + 1}/${toProcess.length}] Processing Sunglasses ${sourceId}...`);
            ...
            try {
                ...
                // Payload — raw name, no description, no name normalisation
    ...
    console.log(`- Total Men's Sunglasses products discovered: ${stats.totalDiscovered}`);
