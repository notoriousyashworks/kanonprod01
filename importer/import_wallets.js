...
// ============================================================
const SOURCE_URL       = 'https://indiankicks.in/shop?c=wallet';
const CATEGORY         = 'Wallets';
const BUNNY_FOLDER     = 'kicks-aura/indiankicks/wallets';
...
async function processVideo(videoFilename, productId) {
    ...
    console.log(`Creating Bunny Stream video for Wallets ${productId}...`);
    const createRes = await fetch(`https://video.bunnycdn.com/library/${BUNNY_STREAM_LIBRARY_ID}/videos`, {
        ...
        body: JSON.stringify({ title: `Wallets_${productId}` })
    });
...
(async () => {
    ...
    while (stats.successfullyImported < MAX_IMPORT && keepDiscovering) {
        ...
        for (let i = 0; i < toProcess.length; i++) {
            ...
            console.log(`\n[${i + 1}/${toProcess.length}] Processing Wallet ${sourceId}...`);
    ...
    console.log(`- Total Wallets discovered: ${stats.totalDiscovered}`);
