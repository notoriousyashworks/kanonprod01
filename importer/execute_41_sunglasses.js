...
(async () => {
    ...
    const toProcess = batchPlan.slice(0, 41);
    ...
    for (const product of toProcess) {
        if (successCount >= 41) {
            console.log("Hard limit of 41 reached. Stopping execution.");
            break;
        }
        ...
        try {
            ...
            for (const imgUrl of sourceImages) {
                ...
                const folderPath = `/kicks-aura/indiankicks/ladies-sunglasses/${product.productId}/`;
            ...
            if (product.video) {
                ...
                if (state.media['video']) {
                     ...
                     try {
                        ...
                        const ikVidUrl = await uploadToImageKit(b64, 'video.mp4', `/kicks-aura/indiankicks/ladies-sunglasses/${product.productId}/`);
    ...
    console.log(`\n================================`);
    console.log(`IMPORT COMPLETE. Successfully created: ${successCount}/41`);
    
    // 7. PUBLIC VISIBILITY VERIFICATION
    console.log(`\nVerifying public visibility (should NOT appear in public filter)...`);
    try {
        const publicReq = await fetch(`${API_URL}/api/v1/products/filter?categories=Ladies%20Sunglasses`);
        ...
        if (!foundAny) {
            console.log(`SUCCESS: None of the newly created products appear in the public Ladies Sunglasses category listing.`);
        }
    } catch (err) {
