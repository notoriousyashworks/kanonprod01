...
async function fetchAllPerfumes(token) {
  const pageSize = 100;
  let allProducts = [];
  let totalPages = 1;
  ...
  for (let page = 0; page < totalPages; page++) {
    ...
    // Exact match: only products whose category is exactly "Perfumes"
    const perfumes = products.filter(p => {
      const cat = (p.category || '').trim();
      return cat === 'Perfumes';
    });

    allProducts = allProducts.concat(perfumes);
    process.stderr.write(`Page ${page+1}/${totalPages}: ${products.length} products, ${perfumes.length} perfumes → total: ${allProducts.length}\n`);
    await sleep(300);
  }
  return allProducts;
}
...
(async () => {
  try {
    ...
    const perfumes = await fetchAllPerfumes(token);
    const slim = perfumes.map(p => ({
      id: p.id,
      name: p.name,
      category: p.category,
    ...
    process.stderr.write(`\nDone. Total "Perfumes" category products: ${perfumes.length}\n`);
  } catch (err) {
