...
async function uploadToBunnyStorage(buffer, filename, folder) {
    // folder expected to be e.g. /kicks-aura/indiankicks/ladies-sunglasses/670774896/
    // Clean up folder slashes
...
(async () => {
    ...
    let foundWithVideo = false;
    let foundWithoutVideo = false;
    ...
    for (const link of linkArray) {
        if (foundWithVideo && foundWithoutVideo) break;
        ...
        if (p) {
            ...
            if (db.imported[p.productId] && db.imported[p.productId].status === 'CREATED') {
                ...
                if (selling > 0 && original > 0 && selling <= original) {
                    ...
                    if (hasVideo && !foundWithVideo) {
                        eligibleProducts.push(p);
                        foundWithVideo = true;
                        console.log(`Selected NEW product WITH video: ${p.productName}`);
                    } else if (!hasVideo && !foundWithoutVideo) {
                        eligibleProducts.push(p);
                        foundWithoutVideo = true;
                        console.log(`Selected NEW product WITHOUT video: ${p.productName}`);
                    }
                }
            }
        }
    ...
    const toProcess = eligibleProducts;
            ...
            for (const imgUrl of sourceImages) {
                ...
                if (state.media[stateKey]) {
                    ...
                    try {
                        ...
                        ikUrls.push(bunnyUrl);
                        state.media[stateKey] = bunnyUrl;
                        saveDb();
