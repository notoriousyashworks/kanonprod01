...
// ============================================================
// CONFIGURATION — Handbags
// ============================================================
const SOURCE_URL       = 'https://indiankicks.in/shop?c=hand-bags';
const CATEGORY         = 'Handbags';
const BUNNY_FOLDER     = 'kicks-aura/indiankicks/handbags';
const MAX_IMPORT       = 257;
...
async function processVideo(videoFilename, productId) {
    ...
    console.log(`Creating Bunny Stream video for Handbags ${productId}...`);
    const createRes = await fetch(`https://video.bunnycdn.com/library/${BUNNY_STREAM_LIBRARY_ID}/videos`, {
        ...
        body: JSON.stringify({ title: `Handbags_${productId}` })
    });
...
(async () => {
    ...
    while (stats.successfullyImported < MAX_IMPORT && keepDiscovering) {
        ...
        for (let i = 0; i < toProcess.length; i++) {
            ...
            console.log(`\n[${i + 1}/${toProcess.length}] Processing Handbag ${sourceId}...`);
    ...
    console.log(`- Total Handbags discovered: ${stats.totalDiscovered}`);
