    ...
    <a href="/product-details?id=${product.id}" class="product-card-link" aria-label="${product.name}">
      <article class="product-card product-card-new" data-product-id="${product.id}">
        <div class="pc-image-wrap">
          ...
          <a href="https://wa.me/916239379751?text=${encodeURIComponent('I am interested in ' + product.name + ' - ' + window.location.origin + '/product-details?id=' + product.id)}" target="_blank" rel="noopener noreferrer" class="pc-whatsapp-btn" onclick="event.stopPropagation();">
