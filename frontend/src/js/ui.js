...
export function openProductWhatsApp(name, id) {
  const number = import.meta.env.VITE_WHATSAPP_NUMBER || '916239379751';
  const text = `I am interested in ${name} - ${window.location.origin}/product-details?id=${id}`;
...
export function createProductCard(product) {
  ...
  if (image) {
    ...
    mediaHTML = `<video 
      poster="${formatVideoPoster(firstVideo)}"
      data-src="${formatVideoHoverPreview(firstVideo)}"
      ...
      <article class="product-card product-card-new ${isHandbag ? 'is-handbag' : ''}" data-product-id="${product.id}">
        ...
        <div class="pc-body">
          ...
          <div class="pc-price-row">
            ...
            ${savingsPct > 0 ? `<span class="pc-savings-badge">-${savingsPct}%</span>` : ''}
          </div>
        </div>
      </article>
    </a>
        ...
        <div class="header__nav-inner">
          ...
          <div class="nav-dropdown-wrapper" id="nav-categories-wrapper">
            ...
            <div class="nav-dropdown" id="nav-categories-dropdown">
              <a href="/products?categories=SNEAKERS">Sneakers</a>
              <a href="/products?categories=STREETWEAR">Streetwear</a>
              <a href="/products?categories=WATCHES">Watches</a>
              <a href="/products?categories=SUNGLASSES">Sunglasses</a>
              <a href="/products?categories=PERFUMES">Perfumes</a>
            </div>
          </div>
    ...
    <footer id="footer" class="footer">
      <div class="container footer__inner">
        ...
        <div class="footer__col" style="align-items: center; justify-content: flex-start; margin-left: auto; margin-top: 18px;">
          <a href="https://instagram.com/kicksaura" target="_blank" rel="noopener" aria-label="Instagram" style="opacity: 0.7; transition: opacity 0.2s;">
