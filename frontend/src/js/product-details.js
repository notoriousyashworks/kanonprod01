...
function renderProduct(product) {
  ...
  const thumbnailsHTML = mediaItems.map((item, i) => {
    if (item.type === 'video') {
      const coverImage = formatVideoPoster(item.url) || images[0] || '';
      return `
  ...
  const whatsappUrl = `https://wa.me/${import.meta.env.VITE_WHATSAPP_NUMBER || '916239379751'}?text=${encodeURIComponent('I am interested in ' + product.name + ' - ' + window.location.origin + '/product-details?id=' + product.id)}`;
  ...
  document.getElementById('product-container').innerHTML = `
    ...
    <div class="product-details">
      ...
      <a href="${whatsappUrl}" target="_blank" rel="noopener noreferrer" class="pd-whatsapp-inquiry-btn">
    ...
    if (drawer && backdrop) {
      ...
      if (chartData) {
        ...
        contentDiv.innerHTML = `
          ...
          <div class="sc-footer">
            Our dedication to craft means that we are committed to getting the right fit. <a href="https://wa.me/${import.meta.env.VITE_WHATSAPP_NUMBER || '916239379751'}?text=Hi!%20I%20want%20help%20in%20finding%20my%20right%20size" target="_blank" rel="noopener">Contact us</a> with questions on how to find the right size.
          </div>`;
      } else {
