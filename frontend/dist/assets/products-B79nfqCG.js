import{k as A,m as _,n as H,j as D,o as Q,p as U,q,x as w,L as N,w as j,t as F,M as W}from"./login-modal.js_v_1-x6Gw3z5f.js";/* empty css                     */import{i as V,a as G}from"./cart-sidebar-CleSB2Zt.js";import{i as K}from"./profile-K0btTpbT.js";document.getElementById("navbar-container").innerHTML=A("products");document.getElementById("footer-container").innerHTML=_();H();D();Q();V();U();K();G();q();const g=0,p=35e3,z=16;let d=0,f=0,P=0,h=[],l=!1;const t={searchQuery:"",categories:[],brands:[],minPrice:g,maxPrice:p,trending:!1};function C(){const e=new URLSearchParams(window.location.search);t.searchQuery=e.get("search")||"",t.categories=e.get("categories")?e.get("categories").split(",").filter(Boolean):[],t.brands=e.get("brands")?e.get("brands").split(",").filter(Boolean):[],t.minPrice=e.has("minPrice")?Number(e.get("minPrice")):g,t.maxPrice=e.has("maxPrice")?Number(e.get("maxPrice")):p,t.trending=e.get("trending")==="true";const n=e.get("category");n&&n!=="all"&&!t.categories.includes(n)&&t.categories.push(n)}function X(){const e=new URLSearchParams;t.searchQuery&&e.set("search",t.searchQuery),t.categories.length&&e.set("categories",t.categories.join(",")),t.brands.length&&e.set("brands",t.brands.join(",")),t.minPrice!==g&&e.set("minPrice",t.minPrice),t.maxPrice!==p&&e.set("maxPrice",t.maxPrice),t.trending&&e.set("trending","true");const n=`${window.location.pathname}${e.toString()?"?"+e.toString():""}`;history.pushState(null,"",n)}window.addEventListener("popstate",()=>{C(),E(),x(),k()});function Z(){const e=document.getElementById("active-chips-row");if(!e)return;const n=[];if(t.categories.forEach(r=>{n.push({label:r,onRemove:()=>{t.categories=t.categories.filter(i=>i!==r),E(),c()}})}),t.brands.forEach(r=>{n.push({label:r,onRemove:()=>{t.brands=t.brands.filter(i=>i!==r),E(),c()}})}),t.trending&&n.push({label:"Trending 🔥",onRemove:()=>{t.trending=!1,c()}}),(t.minPrice!==g||t.maxPrice!==p)&&n.push({label:`₹${t.minPrice.toLocaleString("en-IN")} – ₹${t.maxPrice.toLocaleString("en-IN")}`,onRemove:()=>{t.minPrice=g,t.maxPrice=p,x(),c()}}),n.length===0){e.innerHTML="";return}e.innerHTML=n.map((r,i)=>`
      <button class="filter-chip filter-chip--active" data-chip-index="${i}">
        ...
        <label class="custom-checkbox">
          ...
          <span class="checkmark"></span>
          ${r.name}
        </label>`).join("")}catch(e){console.error("Could not load brands",e)}document.querySelectorAll('#brand-filter-group input[type="checkbox"]').forEach(e=>{e.addEventListener("change",n=>{const r=n.target.dataset.brand;n.target.checked?t.brands.includes(r)||t.brands.push(r):t.brands=t.brands.filter(i=>i!==r),c()})})}function B(){const e=document.getElementById("price-slider-min"),n=document.getElementById("price-slider-max"),r=document.getElementById("price-display"),i=document.querySelector(".range-track");if(!e||!n||!r||!i)return;let a=parseInt(e.value,10),s=parseInt(n.value,10);a>=s&&(a=s-parseInt(e.step,10),e.value=a),r.textContent=`₹${a.toLocaleString("en-IN")} - ₹${s.toLocaleString("en-IN")}`,t.minPrice=a,t.maxPrice=s;const I=a/parseInt(e.max,10)*100,R=s/parseInt(n.max,10)*100;i.style.left=`${I}%`,i.style.width=`${R-I}%`}function x(){const e=document.getElementById("price-slider-min"),n=document.getElementById("price-slider-max");e&&(e.value=t.minPrice),n&&(n.value=t.maxPrice),B()}function te(){const e=document.getElementById("price-slider-min"),n=document.getElementById("price-slider-max");if(!e||!n)return;x();let r;const i=()=>{B(),clearTimeout(r),r=setTimeout(c,400)};e.addEventListener("input",i),n.addEventListener("input",i)}function E(){document.querySelectorAll('#category-filter-group input[type="checkbox"]').forEach(e=>{e.checked=t.categories.includes(e.dataset.cat)}),document.querySelectorAll('#brand-filter-group input[type="checkbox"]').forEach(e=>{e.checked=t.brands.includes(e.dataset.brand)})}function ne(){const e=document.getElementById("nav-search-input");if(!e)return;t.searchQuery&&(e.value=t.searchQuery,e.dispatchEvent(new Event("input",{bubbles:!0})));let n;e.addEventListener("input",()=>{clearTimeout(n),n=setTimeout(()=>{t.searchQuery=e.value.trim(),c()},300)}),e.addEventListener("keydown",r=>{r.key==="Enter"&&(clearTimeout(n),t.searchQuery=e.value.trim(),c())})}let m=null;async function M(){if(m)return m;try{m=(await j()||[]).slice(0,8)}catch{m=[]}return m}let v=!1;async function $(e){if(t.trending){const r=await M();return{content:r,number:0,totalPages:1,totalElements:r.length}}return await N({query:t.searchQuery,categories:t.categories,brands:t.brands,minPrice:t.minPrice,maxPrice:t.maxPrice},e,z)}function L(e){var r;const n=d+1<f;e.innerHTML=`
    ${h.map(w).join("")}
    ${n?`
      <div class="products-view-more">
        <button class="products-view-more-btn" type="button" ${l?"disabled":""}>
          ${l?"Loading...":"View More"}
        </button>
      </div>
    `:""}
  `,(r=e.querySelector(".products-view-more-btn"))==null||r.addEventListener("click",()=>{l||re()})}async function re(){if(l||d+1>=f)return;l=!0;const e=document.getElementById("products-grid");L(e);try{const n=d+1,r=await $(n);h=[...h,...r.content||[]],d=r.number,f=r.totalPages,P=r.totalElements}catch(n){console.error(n)}finally{l=!1,L(e)}}async function k(){if(v)return;v=!0;const e=document.getElementById("products-grid"),n=document.getElementById("results-count");e.innerHTML=`
    ...
    </div>`,J(),Z(),X();try{d=0,h=[];const r=await $(d);if(h=r.content||[],f=r.totalPages||0,P=r.totalElements||0,n.textContent="",P===0){const i=await M(),a=i.length>0?`
          <p class="trending-section-title">Trending Products</p>
          <div class="products-grid">${i.map(w).join("")}</div>
