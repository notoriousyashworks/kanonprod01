import{k as _,m as H,n as D,j as N,o as Q,p as U,q,x as C,L as j,w as F,t as W,M as V}from"./login-modal.js_v_1-vylF9FGw.js";/* empty css                     */import{i as G,a as K}from"./cart-sidebar-CO8nBrVq.js";import{i as z}from"./profile-DqUfOBpi.js";document.getElementById("navbar-container").innerHTML=_("products");document.getElementById("footer-container").innerHTML=H();D();N();Q();G();U();z();K();q();const g=0,p=35e3,X=16;let l=0,f=0,P=0,h=[],d=!1;const t={searchQuery:"",categories:[],brands:[],minPrice:g,maxPrice:p,trending:!1};function T(){const e=new URLSearchParams(window.location.search);t.searchQuery=e.get("search")||"",t.categories=e.get("categories")?e.get("categories").split(",").filter(Boolean):[],t.brands=e.get("brands")?e.get("brands").split(",").filter(Boolean):[],t.minPrice=e.has("minPrice")?Number(e.get("minPrice")):g,t.maxPrice=e.has("maxPrice")?Number(e.get("maxPrice")):p,t.trending=e.get("trending")==="true";const n=e.get("category");n&&n!=="all"&&!t.categories.includes(n)&&t.categories.push(n)}let L=!0;function Z(){const e=new URLSearchParams;t.searchQuery&&e.set("search",t.searchQuery),t.categories.length&&e.set("categories",t.categories.join(",")),t.brands.length&&e.set("brands",t.brands.join(",")),t.minPrice!==g&&e.set("minPrice",t.minPrice),t.maxPrice!==p&&e.set("maxPrice",t.maxPrice),t.trending&&e.set("trending","true");const n=`${window.location.pathname}${e.toString()?"?"+e.toString():""}`;L?(history.replaceState(null,"",n),L=!1):history.pushState(null,"",n)}window.addEventListener("popstate",()=>{L=!0,T(),E(),k(),I()});function J(){const e=document.getElementById("active-chips-row");if(!e)return;const n=[];if(t.categories.forEach(r=>{n.push({label:r,onRemove:()=>{t.categories=t.categories.filter(i=>i!==r),E(),c()}})}),t.brands.forEach(r=>{n.push({label:r,onRemove:()=>{t.brands=t.brands.filter(i=>i!==r),E(),c()}})}),t.trending&&n.push({label:"New Arrivals",onRemove:()=>{t.trending=!1,c()}}),(t.minPrice!==g||t.maxPrice!==p)&&n.push({label:`₹${t.minPrice.toLocaleString("en-IN")} – ₹${t.maxPrice.toLocaleString("en-IN")}`,onRemove:()=>{t.minPrice=g,t.maxPrice=p,k(),c()}}),n.length===0){e.innerHTML="";return}e.innerHTML=n.map((r,i)=>`
    ...
    `:""}
  `,(r=e.querySelector(".products-view-more-btn"))==null||r.addEventListener("click",()=>{d||ie()})}async function ie(){if(d||l+1>=f)return;d=!0;const e=document.getElementById("products-grid");x(e);try{const n=l+1,r=await A(n);h=[...h,...r.content||[]],l=r.number,f=r.totalPages,P=r.totalElements}catch(n){console.error(n)}finally{d=!1,x(e)}}async function I(){if(v)return;v=!0;const e=document.getElementById("products-grid"),n=document.getElementById("results-count");e.innerHTML=`
    ...
    </div>`,Y(),J(),Z();try{l=0,h=[];const r=await A(l);if(h=r.content||[],f=r.totalPages||0,P=r.totalElements||0,n.textContent="",P===0){const i=await $(),a=i.length>0?`
