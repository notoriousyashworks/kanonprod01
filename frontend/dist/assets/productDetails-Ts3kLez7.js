import{k as re,m as ce,n as de,j as Y,o as pe,p as Z,q as ue,B as he,C as me,D as N,A as O,z as G,E as ge,y as ye,F as fe,G as ve,H as be,I as we,J as ke,K as xe,L as Ee}from"./login-modal.js_v_1-HUcqV1jZ.js";/* empty css                     */import{i as Se,a as Le,o as Ue}from"./cart-sidebar-DHRwjtSZ.js";import{i as Ke}from"./profile-B9mEtQiA.js";Ie();document.getElementById("navbar-container").innerHTML=re("product");document.getElementById("footer-container").innerHTML=ce();de();Y();pe();Se();Z();Ke();ue();Le();const W=e=>e!=null?"₹"+e.toLocaleString("en-IN"):"";async function Ie(){const s=new URLSearchParams(window.location.search).get("id");if(!s){Q("No product ID specified in the URL.");return}try{const u=await he(s);$e(u)}catch(u){console.error("Failed to load product:",u),Q("Could not load product. Please try again.")}}function Q(e){document.getElementById("product-container").innerHTML=`
    ...
    `).join("")}
      ...
      ${n}
      ${a}
    `}const x=e.variants&&e.variants.length>0,ee=x?[...e.variants].sort((t,n)=>{const a=parseFloat(String(t.size||"").replace(/[^0-9.]/g,""))||0,r=parseFloat(String(n.size||"").replace(/[^0-9.]/g,""))||0;return a-r}):[],te=x?ee.map(t=>`
    ...
    <div class="product-details">
      ...
      <div class="pd-pricing-section">
        ...
        ${i&&y>0?`<div class="pd-orig-price">${W(i)}</div>`:""}
      </div>
  ...
  `,"IntersectionObserver"in window){const t=new IntersectionObserver((n,a)=>{n.forEach(r=>{if(r.isIntersecting){const l=r.target;l.dataset.initState!=="warmup"&&l.dataset.initState!=="ready"&&l.dataset.initState!=="playing"&&window.initVideoPlayback(l,!0),a.unobserve(l)}})},{rootMargin:"50% 0px 50% 0px"});document.querySelectorAll(".ka-lazy-video").forEach(n=>t.observe(n))}ze(e);let g=0;function L(t){g=t;const n=o[g],a=document.getElementById("img-lightbox"),r=document.getElementById("lightbox-content");if(!a||!r||!n)return;const l=document.querySelector(`.main-media-slide[data-idx="${K}"] video`);l&&l.pause(),r.innerHTML=n.type==="video"?`<div style="position: relative; display: flex; align-items: center; justify-content: center; width: 100%; height: 100%;">
      ...
      </div>`,document.body.appendChild(t),document.getElementById("lightbox-backdrop").addEventListener("click",n=>{n.target===n.currentTarget&&$()}),document.getElementById("lightbox-close").addEventListener("click",$),document.getElementById("lb-prev").addEventListener("click",()=>{g>0&&L(g-1)}),document.getElementById("lb-next").addEventListener("click",()=>{g<o.length-1&&L(g+1)}),document.addEventListener("keydown",n=>{var a;(a=document.getElementById("img-lightbox"))!=null&&a.classList.contains("open")&&(n.key==="Escape"&&$(),n.key==="ArrowLeft"&&g>0&&L(g-1),n.key==="ArrowRight"&&g<o.length-1&&L(g+1))})}const z=e.brand||e.category||"Kicks Aura";if(document.getElementById("size-chart-drawer")){const t=document.getElementById("sc-brand-name");t&&(t.textContent=`Brand : ${z}`)}else{const t=document.createElement("div");t.id="size-chart-drawer",t.innerHTML=`
